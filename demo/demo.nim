...
template newVar(conn : ptr dpiConn, otype : DpiOracleTypes, ntype : DpiNativeCTypes, arrSize : int, colLength : int,
                column : ptr Column) =
    ...
    # example fetching multiple rows 
      onSuccessExecute(context,dpiStmt_execute(prepStatement2,DpiModeExec.DEFAULTMODE.ord, numCols.addr)):
        ...
        for i in countup(1,colcount.int):
          if dpiStmt_getQueryInfo(prepStatement2,i.uint32,colinfo.addr) < DpiResult.SUCCESS.ord:
        ...
        newVar(conn,DpiOracleTypes.OTVARCHAR, DpiNativeCTypes.NativeBYTES,100,10,columns[0].addr)
        if hasError(columns[0]):
          echo "error_newvar_col1"
        newVar(conn,DpiOracleTypes.OTNATIVE_DOUBLE,DpiNativeCTypes.NativeDOUBLE,100,1,columns[1].addr)
        if hasError(columns[1]):
          echo "error_newvar_col2"
        newVar(conn,DpiOracleTypes.OTRAW, DpiNativeCTypes.NativeBYTES,100,10,columns[2].addr)
        if hasError(columns[2]):
          echo "error_newvar_col3"
        newVar(conn,DpiOracleTypes.OTTIMESTAMP_TZ, DpiNativeCTypes.NativeTIMESTAMP,100,1,columns[3].addr)
        if hasError(columns[3]):
          echo "error_newvar_col4"
        
        for i in low(columns) .. high(columns):
          if hasError(context,dpiStmt_define(prepStatement2,(i+1).uint32,columns[i].colVar)):
              var err  : dpiErrorInfo
              dpiContext_getError(context,err.addr)
              break
        ...
        onSuccessExecute(context,dpiStmt_fetchRows(prepStatement2,
          ...
          # tstamp column
          var tstamp : dpiTimestamp = cast[dpiTimestamp](columns[3].buffer.value.asTimestamp)
          echo "col4: " & $tstamp

          if isNull(columns[3].buffer[1]):
            echo "col4: col missing value "
          else:
            tstamp = cast[dpiTimestamp](columns[3].buffer[1].value.asTimestamp)
            echo "col4: " & $tstamp
